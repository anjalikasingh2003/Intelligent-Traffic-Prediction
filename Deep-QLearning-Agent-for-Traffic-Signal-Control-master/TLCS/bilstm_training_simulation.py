...
class Simulation:
    def __init__(self, Model, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs, predictor=None):
        self._Model = Model
        self._Memory = Memory
        ...
        self._reward_store = {"SUMO": [], "BiLSTM": []}
        self._cumulative_wait_store = {"SUMO": [], "BiLSTM": []}
        self._avg_queue_length_store = {"SUMO": [], "BiLSTM": []}
        ...
        self.predictions = []  # Store (prediction, actual) pairs for evaluation

    def train_with_batch(self):
        """Train predictor with accumulated data in batches"""
        if not self.predictor or len(self.sequence_memory) < 32:  # Wait for enough samples
            return
        ...
        # Train model
        self.predictor.model.fit(X, y, epochs=3, batch_size=min(32, len(batch_indices)), verbose=0)
    ...
    def run(self, episode, epsilon, model_type):
        ...
        os.makedirs('bilstm_csv', exist_ok=True)
        original_csv_path = os.path.join('csv', f"states_episode_{episode}.csv")
        bilstm_csv_path = os.path.join('bilstm_csv', f"bilstm_episode_{episode}.csv")
        
        with open(original_csv_path, 'w') as orig_file, open(bilstm_csv_path, 'w') as bilstm_file:
            orig_writer = csv.writer(orig_file)
            bilstm_writer = csv.writer(bilstm_file)
            
            # Write headers
            orig_writer.writerow(["timestamp", "states", "source"])
            bilstm_writer.writerow(["timestamp", "states", "source"])
            
            # Initialize simulation with yellow and green phases
            self._simulate(self._yellow_duration, orig_writer, bilstm_writer)
            self._simulate(self._green_duration, orig_writer, bilstm_writer)
            ...
            while self._step < self._max_steps:
                ...
                if model_type == "BiLSTM" and len(self.state_buffer) >= self.min_buffer_size:
                    if self._step % self.prediction_interval == 0:  # Predict every 10th step
                        # Use BiLSTM prediction
                        predicted_state = self.predictor.predict(np.array(self.state_buffer))
                        current_state = predicted_state
                        source = "BiLSTM"
                        
                        # Store this prediction and the future actual state for evaluation
                        self.predictions.append((predicted_state, None))  # Will fill actual state later
                    ...
                    # If we're at step+1 after a prediction, get actual outcome and store in memory
                        ...
                        if self.predictions:
                            self.predictions[-1] = (self.predictions[-1][0], actual_state)
                        ...
                        if len(self.predictions) % 5 == 0:  # Every 5 predictions
                            self.train_with_batch()
                        
                # Write to CSVs
                self._write_to_csv(self._step, sumo_state, current_state, source, orig_writer, bilstm_writer)
    ...
    def _simulate(self, steps_todo, orig_writer, bilstm_writer):
        """
    ...
    def _write_to_csv(self, step, sumo_state, bilstm_state, source, orig_writer, bilstm_writer):
        """
        Writes state information to the respective CSV files.
        - orig_writer logs all SUMO states.
        - bilstm_writer logs 9 SUMO states + 1 BiLSTM-predicted state every 10th step.
        """
        # Write all SUMO states to original CSV
        orig_writer.writerow([step, list(sumo_state), "SUMO"])

        # Write to BiLSTM CSV based on the source (SUMO or BiLSTM)
        if source == "SUMO":
            bilstm_writer.writerow([step, list(sumo_state), "SUMO"])
        elif source == "BiLSTM":
            bilstm_writer.writerow([step, list(bilstm_state), "BiLSTM"])


    def pre_train_predictor(self, episodes=5):
        """Pre-train predictor model on collected sequences from initial episodes"""
        print("Starting pre-training...")
        if not self.predictor:
            return  # Only run if we have a predictor
            
        print("Pre-training BiLSTM model...")
        collected_sequences = []
        collected_targets = []
        ...
        self.predictor.model.fit(X, y, epochs=10, batch_size=32, verbose=1)
        print("Pre-training complete")


    def evaluate_predictor_accuracy(self):
        """Calculate and return prediction accuracy metrics"""
        if not hasattr(self, 'predictions') or not self.predictions:
            return {}
        
        # Filter out entries with None actual values
        valid_predictions = [(pred, actual) for pred, actual in self.predictions if actual is not None]
