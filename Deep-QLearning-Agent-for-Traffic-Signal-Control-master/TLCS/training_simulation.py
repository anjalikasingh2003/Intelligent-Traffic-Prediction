...
class Simulation:
    def __init__(self, Model, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs, lstm_predictor):
        self._Model = Model
        self._Memory = Memory
        ...
        self._reward_store = {"SUMO": [], "LSTM": []}
        self._cumulative_wait_store = {"SUMO": [], "LSTM": []}
        self._avg_queue_length_store = {"SUMO": [], "LSTM": []}
        self._training_epochs = training_epochs
        self._sum_queue_length = 0  # Initialize _sum_queue_length
        self._sum_waiting_time = 0  # Initialize _sum_waiting_time
        self.lstm_predictor = lstm_predictor
        self.state_buffer = []  # Buffer to store last 9 SUMO states
        self.lstm_counter = 0  # Counter to track prediction intervals
        self.min_buffer_size=9
        self.prediction_interval = 10
        self.sequence_memory = []  # Add this: Store (sequence, actual_next_state) pairs
        self.lstm_predictions = []  # Add this: Store (prediction, actual) pairs for evaluation

    def train_lstm_with_batch(self):
        """Train LSTM with accumulated data in batches"""
        if not self.lstm_predictor or len(self.sequence_memory) < 32:  # Wait for enough samples
            return
        ...
        # Train model
        self.lstm_predictor.model.fit(X, y, epochs=3, batch_size=min(32, len(batch_indices)), verbose=0)
    ...
    def run(self, episode, epsilon, model_type):
        ...
        lstm_csv_path = os.path.join('lstm_csv', f"lstm_episode_{episode}.csv")
        
        with open(original_csv_path, 'w') as orig_file, open(lstm_csv_path, 'w') as lstm_file:
            orig_writer = csv.writer(orig_file)
            lstm_writer = csv.writer(lstm_file)
            
            # Write headers
            orig_writer.writerow(["timestamp", "states", "source"])
            lstm_writer.writerow(["timestamp", "states", "source"])
            
            # Initialize simulation with yellow and green phases
            self._simulate(self._yellow_duration, orig_writer, lstm_writer)
            self._simulate(self._green_duration, orig_writer, lstm_writer)
            ...
            while self._step < self._max_steps:
                ...
                if model_type == "LSTM" and len(self.state_buffer) >= self.min_buffer_size:
                    if self._step % self.prediction_interval == 0:  # Predict every 10th step
                        # Use LSTM prediction
                        lstm_state = self.lstm_predictor.predict(np.array(self.state_buffer))
                        current_state = lstm_state
                        source = "LSTM"
                        
                        # Store this prediction and the future actual state for evaluation
                        self.lstm_predictions.append((lstm_state, None))  # Will fill actual state later
                    ...
                    # If we're at step+1 after a prediction, get actual outcome and store in memory
                        ...
                        if self.lstm_predictions:
                            self.lstm_predictions[-1] = (self.lstm_predictions[-1][0], actual_state)
                        ...
                        if len(self.lstm_predictions) % 5 == 0:  # Every 5 predictions
                            self.train_lstm_with_batch()
                        
                # Write to CSVs
                self._write_to_csv(self._step, sumo_state, current_state, source, orig_writer, lstm_writer)
    ...
    def _simulate(self, steps_todo, orig_writer, lstm_writer):
        """
    ...
    #     """
    #     Train the LSTM predictor on collected state sequences
    #     """
    #     if not self.lstm_predictor or len(self.state_buffer) < self.min_buffer_size + 1:
    #         return
            
    #     # Create training data from collected state sequences
    #     X = []
    #     y = []
    #     for i in range(len(self.state_buffer) - self.min_buffer_size):
    #         X.append(self.state_buffer[i:i+self.min_buffer_size])
    #         y.append(self.state_buffer[i+self.min_buffer_size])
    ...
    #     self.lstm_predictor.model.fit(X, y, epochs=5, batch_size=32, verbose=0)


    def _save_episode_stats(self, model_type):
        """
    ...
    def _write_to_csv(self, step, sumo_state, lstm_state, source, orig_writer, lstm_writer):
        """
        Writes state information to the respective CSV files.
        - orig_writer logs all SUMO states.
        - lstm_writer logs 9 SUMO states + 1 LSTM-predicted state every 10th step.
        """
        # Write all SUMO states to original CSV
        orig_writer.writerow([step, list(sumo_state), "SUMO"])

        # Write to LSTM CSV based on the source (SUMO or LSTM)
        if source == "SUMO":
            lstm_writer.writerow([step, list(sumo_state), "SUMO"])
        elif source == "LSTM":
            lstm_writer.writerow([step, list(lstm_state), "LSTM"])


    def pre_train_lstm(self, episodes=5):
        """Pre-train LSTM model on collected sequences from initial episodes"""
        ...
        self.lstm_predictor.model.fit(X, y, epochs=10, batch_size=32, verbose=1)
        print("Pre-training complete")


    def evaluate_lstm_accuracy(self):
        """Calculate and return LSTM prediction accuracy metrics"""
        if not hasattr(self, 'lstm_predictions') or not self.lstm_predictions:
            return {}
        
        # Filter out entries with None actual values
        valid_predictions = [(pred, actual) for pred, actual in self.lstm_predictions if actual is not None]
