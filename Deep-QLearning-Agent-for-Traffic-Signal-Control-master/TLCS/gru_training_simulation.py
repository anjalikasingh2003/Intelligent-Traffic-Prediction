...
class Simulation:
    def __init__(self, Model, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs, gru_predictor):
        self._Model = Model
        self._Memory = Memory
        ...
        self._reward_store = {"SUMO": [], "GRU": []}
        self._cumulative_wait_store = {"SUMO": [], "GRU": []}
        self._avg_queue_length_store = {"SUMO": [], "GRU": []}
        ...
        self.gru_predictions = []  # Store (prediction, actual) pairs for evaluation

    def train_gru_with_batch(self):
        """Train GRU with accumulated data in batches"""
        if not self.gru_predictor or len(self.sequence_memory) < 32:  # Wait for enough samples
            return
        ...
        # Train model
        self.gru_predictor.model.fit(X, y, epochs=3, batch_size=min(32, len(batch_indices)), verbose=0)
    ...
    def run(self, episode, epsilon, model_type):
        ...
        gru_csv_path = os.path.join('gru_csv', f"gru_episode_{episode}.csv")
        
        with open(original_csv_path, 'w') as orig_file, open(gru_csv_path, 'w') as gru_file:
            orig_writer = csv.writer(orig_file)
            gru_writer = csv.writer(gru_file)
            
            # Write headers
            orig_writer.writerow(["timestamp", "states", "source"])
            gru_writer.writerow(["timestamp", "states", "source"])
            
            # Initialize simulation with yellow and green phases
            self._simulate(self._yellow_duration, orig_writer, gru_writer)
            self._simulate(self._green_duration, orig_writer, gru_writer)
            ...
            while self._step < self._max_steps:
                ...
                if model_type == "GRU" and len(self.state_buffer) >= self.min_buffer_size:
                    if self._step % self.prediction_interval == 0:  # Predict every 10th step
                        # Use GRU prediction
                        gru_state = self.gru_predictor.predict(np.array(self.state_buffer))
                        current_state = gru_state
                        source = "GRU"
                        
                        # Store this prediction and the future actual state for evaluation
                        self.gru_predictions.append((gru_state, None))  # Will fill actual state later
                    ...
                    # If we're at step+1 after a prediction, get actual outcome and store in memory
                        ...
                        if self.gru_predictions:
                            self.gru_predictions[-1] = (self.gru_predictions[-1][0], actual_state)
                        ...
                        if len(self.gru_predictions) % 5 == 0:  # Every 5 predictions
                            self.train_gru_with_batch()
                        
                # Write to CSVs
                self._write_to_csv(self._step, sumo_state, current_state, source, orig_writer, gru_writer)
    ...
    def _simulate(self, steps_todo, orig_writer, gru_writer):
        """
    ...
    def _write_to_csv(self, step, sumo_state, gru_state, source, orig_writer, gru_writer):
        """
        Writes state information to the respective CSV files.
        - orig_writer logs all SUMO states.
        - gru_writer logs 9 SUMO states + 1 GRU-predicted state every 10th step.
        """
        # Write all SUMO states to original CSV
        orig_writer.writerow([step, list(sumo_state), "SUMO"])

        # Write to GRU CSV based on the source (SUMO or GRU)
        if source == "SUMO":
            gru_writer.writerow([step, list(sumo_state), "SUMO"])
        elif source == "GRU":
            gru_writer.writerow([step, list(gru_state), "GRU"])


    def pre_train_gru(self, episodes=5):
        """Pre-train GRU model on collected sequences from initial episodes"""
        ...
        self.gru_predictor.model.fit(X, y, epochs=10, batch_size=32, verbose=1)
        print("Pre-training complete")


    def evaluate_gru_accuracy(self):
        """Calculate and return GRU prediction accuracy metrics"""
        if not hasattr(self, 'gru_predictions') or not self.gru_predictions:
            return {}
        
        # Filter out entries with None actual values
        valid_predictions = [(pred, actual) for pred, actual in self.gru_predictions if actual is not None]
